        ...
        <ul className="space-y-1.5 sm:space-y-2">
            {items.map((item, index) => (
                <li key={index} className="flex items-start">
                    <svg className="w-3 h-3 sm:w-4 sm:h-4 text-brand-secondary mt-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path>
                    </svg>
                    <p className="ml-2 text-xs sm:text-sm text-gray-700 dark:text-gray-300">{item}</p>
                </li>
            ))}
        </ul>
    </div>
    ...
    return (
        <div className="p-3 sm:p-4 md:p-6">
            <div className="mb-4 sm:mb-6">
                <h2 className="text-xl sm:text-2xl font-bold text-brand-dark dark:text-white mb-2">Document History</h2>
                                ...
                                >
                                    <div className="flex justify-between items-start gap-2">
                                        <div className="flex-1 min-w-0">
                                            <h4 className="font-semibold text-sm sm:text-base text-brand-dark dark:text-white truncate">
                                                {doc.file_name}
                                            </h4>
                                            <div className="mt-1.5 sm:mt-2 flex flex-wrap gap-1.5 sm:gap-2 text-xs text-gray-500 dark:text-gray-400">
                                                <span>{formatFileSize(doc.file_size)}</span>
                                                <span>•</span>
                                                <span className="hidden sm:inline">{formatDate(doc.created_at)}</span>
                                                <span className="sm:hidden">{new Date(doc.created_at).toLocaleDateString()}</span>
                                            </div>
                                            {doc.summary && (
                                        ...
                                        <button
                                            ...
                                            className="ml-2 p-1.5 sm:p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg disabled:opacity-50 flex-shrink-0"
                        ...
                        {selectedDoc ? (
                            <div className="space-y-4 sm:space-y-6">
                                <div className="p-4 sm:p-6 bg-white dark:bg-slate-800 rounded-lg shadow-md">
                                    <h2 className="text-lg sm:text-xl font-bold text-brand-dark dark:text-white mb-2 sm:mb-3 break-words">
                                ...
                                {selectedDoc.file_url && (
                                    <div className="p-3 sm:p-4 bg-gray-50 dark:bg-slate-700 rounded-lg flex flex-col sm:flex-row gap-2 sm:gap-3">
                                        <a
                                            href={selectedDoc.file_url}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="flex items-center justify-center px-3 sm:px-4 py-2 text-xs sm:text-sm bg-brand-secondary text-white rounded-lg hover:bg-sky-600 transition-colors"
                                        >
                                        ...
                                        <a
                                            ...
                                            className="flex items-center justify-center px-3 sm:px-4 py-2 text-xs sm:text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"
                                        >
